...
SCENARIO("Service detail state handling") {
    ...
    GIVEN("Bad Case - Unknown session") {
        ...
        req.service = datatypes::ServiceCategory::DC;
        ...
        THEN("ResponseCode: FAILED_UnknownSession, mandatory fields should be set") {
            ...
            REQUIRE(res.service == datatypes::ServiceCategory::DC);
            ...
            REQUIRE(static_cast<datatypes::DcConnector>(connector) == datatypes::DcConnector::Core);
        }
    }
    ...
    GIVEN("Bad Case - FAILED_ServiceIDInvalid") {
        ...
        req.header.session_id = session.get_id();
        req.header.timestamp = 1691411798;
        req.service = datatypes::ServiceCategory::AC;
        ...
        THEN("ResponseCode: FAILED_ServiceIDInvalid, mandatory fields should be set") {
            REQUIRE(res.response_code == datatypes::ResponseCode::FAILED_ServiceIDInvalid);
            REQUIRE(res.service == datatypes::ServiceCategory::DC);
            REQUIRE(res.service_parameter_list.size() == 1);
            auto& parameter = res.service_parameter_list[0];
            REQUIRE(parameter.id == 0);
    ...
    GIVEN("Good Case - DC_BPT Service") {
        ...
        req.header.session_id = session.get_id();
        req.header.timestamp = 1691411798;
        req.service = datatypes::ServiceCategory::DC_BPT;

        const auto res = d20::state::handle_request(req, session, session_config);

        THEN("ResponseCode: OK") {
            REQUIRE(res.response_code == datatypes::ResponseCode::OK);
            REQUIRE(res.service == datatypes::ServiceCategory::DC_BPT);
            REQUIRE(res.service_parameter_list.size() == 1);
            auto& parameters = res.service_parameter_list[0];
            REQUIRE(parameters.id == 0);
    ...
    GIVEN("Good Case - 2x DC Services") {
        ...
        session_config.dc_parameter_list = {{
                                            ...
                                            {
                                                datatypes::DcConnector::Extended,
                                                datatypes::ControlMode::Dynamic,
                                                datatypes::MobilityNeedsMode::ProvidedBySecc,
                                                datatypes::Pricing::NoPricing,
                                            }};
    ...
    GIVEN("Good case - Internet service") {
        ...
        auto session_config = d20::SessionConfig(evse_setup);
        session_config.internet_parameter_list = {{datatypes::Protocol::Http, datatypes::Port::Port80}};
        session_config.dc_parameter_list = {{
            datatypes::DcConnector::Extended,
            datatypes::ControlMode::Scheduled,
            datatypes::MobilityNeedsMode::ProvidedByEvcc,
        ...
        req.header.session_id = session.get_id();
        req.header.timestamp = 1691411798;
        req.service = datatypes::ServiceCategory::Internet;

        const auto res = d20::state::handle_request(req, session, session_config);

        THEN("ResponseCode: OK") {
            REQUIRE(res.response_code == datatypes::ResponseCode::OK);
            REQUIRE(res.service == datatypes::ServiceCategory::Internet);
            REQUIRE(res.service_parameter_list.size() == 1);
            auto& parameters = res.service_parameter_list[0];
    ...
    GIVEN("Good case - Parking status service") {
        d20::Session session = d20::Session();
        session.offered_services.energy_services = {datatypes::ServiceCategory::DC};
        session.offered_services.vas_services = {datatypes::ServiceCategory::ParkingStatus};

        auto session_config = d20::SessionConfig(evse_setup);
        session_config.parking_parameter_list = {
            {datatypes::IntendedService::VehicleCheckIn, datatypes::ParkingStatus::ManualExternal}};
        session_config.dc_parameter_list = {{
            datatypes::DcConnector::Extended,
            datatypes::ControlMode::Scheduled,
            datatypes::MobilityNeedsMode::ProvidedByEvcc,
        ...
        req.header.session_id = session.get_id();
        req.header.timestamp = 1691411798;
        req.service = datatypes::ServiceCategory::ParkingStatus;

        const auto res = d20::state::handle_request(req, session, session_config);

        THEN("ResponseCode: OK") {
            REQUIRE(res.response_code == datatypes::ResponseCode::OK);
            REQUIRE(res.service == datatypes::ServiceCategory::ParkingStatus);
            REQUIRE(res.service_parameter_list.size() == 1);
            auto& parameters = res.service_parameter_list[0];
            REQUIRE(parameters.id == 0);
            REQUIRE(parameters.parameter.size() == 2);

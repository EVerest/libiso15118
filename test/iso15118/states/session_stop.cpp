...
SCENARIO("Session Stop state handling") {
    ...
    GIVEN("Bad case - Unknown session") {
        ...
        req.charging_session = datatypes::ChargingSession::Terminate;
    ...
    GIVEN("Bad case - FAILED_NoServiceRenegotiationSupported") {
        ...
        req.header.session_id = session.get_id();
        req.header.timestamp = 1691411798;
        req.charging_session = datatypes::ChargingSession::ServiceRenegotiation;

        const auto res = d20::state::handle_request(req, session);

        THEN("ResponseCode: OK") {
            REQUIRE(res.response_code == datatypes::ResponseCode::FAILED_NoServiceRenegotiationSupported);
        }
    }

...
bool validate_and_setup_header(message_20::Header& header, const states::Session& cur_session,
...
void setup_header(message_20::Header& header, const states::Session& cur_session) {
    header.session_id = cur_session.get_id();
    setup_timestamp(header);
}

template <typename Response> Response handle_sequence_error(const states::Session& session) {
    Response res;
...
void send_sequence_error(const message_20::Type req_type, states::Context& ctx) {

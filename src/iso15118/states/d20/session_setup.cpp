...
#include <iso15118/states/d20/authorization_setup.hpp>
...
states::FsmSimpleState::HandleEventReturnType SessionSetup::handle_event(AllocatorType& sa, states::FsmEvent ev) {

    if (ev != states::FsmEvent::V2GTP_MESSAGE) {
        return sa.PASS_ON;
    }

    const auto variant = ctx.get_request<message_20::Variant>();

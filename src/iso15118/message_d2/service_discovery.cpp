...
template <> void convert(const struct iso2_ServiceDiscoveryReqType& in, ServiceDiscoveryRequest& out) {
    ...
    // RDB ISO2 is somewhat different than ISO2, with just ServiceScope and ServiceCategory instead of
    // SupportedServiceIDs for filtering the response, though maybe they can be mapped so we can use the iso20 code in
    // the state handler? RDB TODO ignored for the moment
...
template <> void convert(const ServiceDiscoveryResponse& in, iso2_ServiceDiscoveryResType& out) {
    ...
    for (auto const& service : in.energy_transfer_service_list) {
        if (service.service_id == ServiceCategory::DC) {
            ...
            out_service.SupportedEnergyTransferMode.EnergyTransferMode.array[0] =

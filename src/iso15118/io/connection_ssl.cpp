...
static std::tuple<int, sockaddr_in6> create_udp_socket(const char* interface_name, uint16_t port) {
    ...
    if (udp_socket < 0) {
        ...
        logf(LogLevel::Error, error_msg.c_str());
        return {udp_socket, {}};
    }
    ...
    logf(LogLevel::Info, "UDP socket bound to source port: %u", source_port);
...
void ConnectionSSL::init_ssl(const config::SSLConfig& ssl_config) {
    ...
    if (ssl_config.enable_ssl_logging) {
        ...
        SSL_CTX_set_keylog_callback(ctx, [](const SSL* ssl, const char* line) {
            logf(LogLevel::Info, "TLS handshake keys: %s\n", line);

            if (key_log_fd == -1) {
                logf(LogLevel::Warning, "Error - key_log_fd is not available");
                return;
            }
...
void ConnectionSSL::handle_connect() {
    ...
    logf(LogLevel::Info, "Incoming connection from [%s]:%s", ip, service);
...
void ConnectionSSL::handle_data() {
    if (not handshake_complete) {
        ...
        if (ssl_handshake_result <= 0) {
            ...
            logf(LogLevel::Info, "Handshake complete!\n");
...
void ConnectionSSL::close() {
    ...
    logf(LogLevel::Info, "Closing TLS connection\n");
